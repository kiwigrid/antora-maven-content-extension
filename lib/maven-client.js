...
class MavenClient {
    ...
    constructor(logger, {maxRetries = 3, backoff = 300} = {}) {
        this.logger = logger
        this.maxRetries = maxRetries;
        this.backoff = backoff;
    }

    async #singleFetch(url, options = {}, redirectCount = 0) {
        return new Promise((resolve, reject) => {
            ...
            const req = client.request(requestOptions, (response) => {
                ...
                if ((response.statusCode === 301 || response.statusCode === 302) && response.headers.location) {
                    ...
                    this.#singleFetch(redirectUrl.href, options, redirectCount + 1).then(resolve).catch(reject);
                    return;
                }
    ...
    async #nativeFetchWithRetry(url, options = {}) {
        let attempt = 0;
        let failReasons = []
        while (attempt < this.maxRetries) {
            try {
                const response = await this.#singleFetch(url, options);
                if (response.ok || response.status >= 400 && response.status < 500) {
                    return response;
                }
                failReasons.push(`${attempt + 1}: Status(${response.status})`)
                this.logger.warn(`Request to ${url} failed with status ${response.status}. Retrying...`);
            } catch (error) {
                failReasons.push(`${attempt + 1}: Error(${error.message})`)
                this.logger.warn(`Request to ${url} failed: ${error.message}. Retrying...`);
            }
            attempt++;
        ...
        throw new MavenClientError(`Request to ${url} failed after ${this.maxRetries} attempts:\n${failReasons.join('\n')}\n`);
    }
    ...
    async downloadAndExtract(repository, targetFolder, mavenArtifact) {
        ...
        const response = await this.#nativeFetchWithRetry(downloadUrl, repository.fetchOptions);
        if (!response.ok) {
    ...
    async #buildSnapshotUrl(repository, mavenArtifact) {
        ...
        if (!latestVersion) {
            throw new MavenClientError(`Cannot find latest snapshot version info for ${mavenArtifact.extension} in maven metadata: ${snaphotMetadataUrl}`)
        }
    ...
    async #downloadXml(url, fetchOptions) {
        let metaDataResponse = await this.#nativeFetchWithRetry(url, fetchOptions)
    ...
    async #existsRemotely(url, fetchOptions) {
        ...
        options.method = 'HEAD';
        let probeResponse = await this.#nativeFetchWithRetry(url, options);
