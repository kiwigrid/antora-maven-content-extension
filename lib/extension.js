...
class MavenContentSourceExtension {
     ...
     * @param config Extension configuration. note that antora preprocesses (normalizes) the config, that's why several variants need to be checked because the preprocessing does different things in different versions.
     */
    ...
    async onPlaybookBuilt({playbook}) {
        ...
        if (this.config.mavenSettings || this.config.mavensettings) {
            const settingsFile = (this.config.mavenSettings === true || this.config.mavensettings === true)
                ? this.mavenClient.findMavenSettingsFile()
                : (this.config.mavenSettings || this.config.mavensettings)
            ;
